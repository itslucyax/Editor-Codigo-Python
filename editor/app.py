...
class EditorApp(tk.Tk):
        ...
        def __init__(
            ...
            # Atajo para renombrar variable (F2)
            self.bind_all("<F2>", self._renombrar_variable)
            # ...existing code...

        def _renombrar_variable(self, event=None):
            """Renombra todas las apariciones de la palabra bajo el cursor (F2)."""
            # Intentar obtener la palabra seleccionada o la que está bajo el cursor
            try:
                if self.text_editor.tag_ranges("sel"):
                    old_name = self.text_editor.get("sel.first", "sel.last").strip()
                else:
                    old_name = self.text_editor.get("insert wordstart", "insert wordend").strip()
            except:
                return "break"
            ...
            # Uso de Regex con \b para asegurar que solo cambie la palabra completa
            # El flag re.IGNORECASE es vital porque VBS es case-insensitive
            import re
            nuevo_contenido = re.sub(rf"\b{re.escape(old_name)}\b", new_name, contenido, flags=re.IGNORECASE)

            # Si hubo cambios, aplicarlos
            if nuevo_contenido != contenido:
                self.text_editor.set_content(nuevo_contenido)
                self.text_editor._user_modified = True
                self._update_status()
                self.status_var.set(f"✓ Variable '{old_name}' renombrada a '{new_name}'")
