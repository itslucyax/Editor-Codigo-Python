...
class DatabaseConnection:
    ...
    def get_record_full(self, key_columns, key_values):
        """Carga el registro completo para el editor"""
        try:
    ...
    def get_scripts_for_search(self):
        """Para el buscador: trae todos los scripts del mismo modelo"""
        try:
            sql = f"SELECT CODIGO, SCRIPT FROM [{self.table}] WHERE MODELO = ?"
            cur = self._cursor()
            return cur.execute(sql, [self.modelo]).fetchall()
        except Exception as e:
            logger.error(f"Error en búsqueda por modelo: {e}")
            return []

    def get_acum_fields(self):
        """Para el buscador de campos ACUM de la tabla maestra E_ACUM"""
        try:
            cur = self._cursor()
            return cur.execute("SELECT ACUM, DESCRI FROM E_ACUM ORDER BY ACUM").fetchall()
        except Exception as e:
            logger.error(f"Error cargando E_ACUM: {e}")
            return []
        """
